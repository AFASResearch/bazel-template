...
{
  internal interface INugetRepositoryEntry
  {
    string Source { get; }
  ...
  internal class NugetRepositoryEntry : INugetRepositoryEntry
  {
    ...
    public string Source => ReadSource();
    ...
    {
      var nupkgMetadataPath = LocalPackageSourceInfo.Repository.PathResolver.GetNupkgMetadataPath(LocalPackageSourceInfo.Package.Id, LocalPackageSourceInfo.Package.Version);
      var source = JObject.Parse(File.ReadAllText(nupkgMetadataPath))["source"]?.Value<string>();

      if(string.Equals(source, "https://pkgs.dev.azure.com/dnceng/public/_packaging/dotnet-public/nuget/v3/index.json"))
      {
        return "https://api.nuget.org/v3/index.json";
      }

      // This could resolve some system paths
      return (source != null && source.StartsWith("http")) ? source : "https://api.nuget.org/v3/index.json";
    }
  }
}