        ...
        public static void Main(string[] args)
        {
            ...
            void CreateTargets(WorkRequest request)
            {
                ...
                string ScopePath(string path)
                {
                    ...
                    {
                        // the real external folder. Not symlinked
                ...
                var outFolder = Path.GetDirectoryName(request.Arguments[1]).Replace('/', '\\') + "\\";

                var sb = new StringBuilder();

                foreach(var line in request.Arguments.Skip(2))
                {
                    // compile files
                    if(!line.StartsWith('/'))
                    {
...
");
                    ...
                    {
                        var rulesetPath = line.Substring(9).Replace('/', '\\');
                        sb.Append($"</ItemGroup><PropertyGroup><CSCRuleSet>{ScopePath(rulesetPath)}</CSCRuleSet></PropertyGroup><ItemGroup>\n\n");
                    }
                }
                ...
                var (to, from) = SplitBin(request.Arguments[1]);
                ...
                var persistedProps = Path.GetFullPath(Path.ChangeExtension(request.Arguments[1], ".persist.props"));
...
");
                ...
                File.WriteAllText(request.Arguments[1], string.Empty);
...
");
            ...
            async Task Compile(WorkRequest request)
            {
                ...
                var lines = File.ReadAllLines(cscParamsFile);
                var language = lines
                  .Reverse()
                  .Where(l => l[0] != '/')
                  .Any(l => l.EndsWith(".cs") || l.EndsWith(".cs\""))
                    ? RequestLanguage.CSharpCompile 
                    : RequestLanguage.VisualBasicCompile;
                var response = await client.Work(request.RequestId, language, cscParamsFile, cancelSource.Token).ConfigureAwait(false);

                if (response.ExitCode == 0 && request.Arguments.Count >= 4)
                {
                    var unusedRefsOutput = request.Arguments[2];
                    var dll = request.Arguments[3];
                    var unusedReferences = ResolveUnusedReferences(lines, dll);
        ...
        private static IEnumerable<string> ResolveUnusedReferences(string[] lines, string dll)
        {
