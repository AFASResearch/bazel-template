...
Unfortunately IDE's do not allow overwriting a Build All target using MSBUILD logic. This results in IDE's recursively building all projects themselves potentially resulting in many calls to bazel. For this we have built a Rider/ReSharper plugin, which drastically speeds up builds by only calling Bazel once. You can find the plugin here:
ReSharper: https://plugins.jetbrains.com/plugin/15130-bazelbuild
Rider: https://plugins.jetbrains.com/plugin/15155-bazelbuild
...
* We rely on [dotnet startup hooks](https://github.com/dotnet/runtime/blob/main/docs/design/features/host-startup-hook.md) for runtime assembly loading using the `runfiles_manifest`.
