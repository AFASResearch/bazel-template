...
load(
    "@rules_dotnet//dotnet/private:rids.bzl",
    "RUNTIME_GRAPH",
)
load(
    "@rules_dotnet//dotnet/private:providers.bzl",
    "TargettingPackOverrides",
)
...
def _process_lib_file(groups, file, is_build = False):
    i = file.find("/")
    tfm_start = i + 1
    ...
    if file.find("/", tfm_end + 1) != -1:
        if not (is_build or file.endswith(".resources.dll")):
            return
...
def _process_analyzer_file(groups, file):
    ...
    g = ""
    if file.startswith("analyzers/dotnet/roslyn"):
        g = file[len("analyzers/dotnet/"):file.find("/", len("analyzers/dotnet/"))]

    group = groups["analyzers"]["dotnet"]
...
def _process_key_and_file(name, groups, key, file):
    # todo resource dlls
    if key == "lib":
        _process_lib_file(groups, file)
    elif key == "ref":
        _process_ref_file(groups, file)
    elif key == "build" and name.find("servicefabric.actors") == -1: # netstandard.library & nunit3testadapter cases
        _process_lib_file(groups, file, True)
    elif key == "analyzers":
...
def _nuget_archive_impl(ctx):
    ...
    # if overrides.exists:
    #     fail({ l[0]: ([int(v) for v in l[1].split(".")]) for l in  })
    ...
    if groups.get("ref") and groups.get("lib"):
        ...
        # sometimes refs only contains _._ files and libs should be used alternatively
        for (tfm, lib) in groups.get("lib").items():
            if tfm not in refs:
                refs[tfm] = lib

    # workaround when a netX app transitively depends on netstandard
    if ctx.attr.id == "netstandard.library":
        groups["lib"]["netcoreapp1.0"] = []

    overrides = ctx.path("data/PackageOverrides.txt")
    manifest = ctx.path("data/PlatformManifest.txt")
    overrides_obj = {}

    if overrides.exists:
        for l in ctx.read(overrides).splitlines():
            s = l.split('|')
            overrides_obj[s[0]] = s[1]
    ...
    analyzers = groups["analyzers"]["dotnet"].get("", [])
    # if there are roslyn specific groups add the highest
    l = len(groups["analyzers"]["dotnet"].items())
    if l > 1:
        analyzers.extend(groups["analyzers"]["dotnet"].values()[l-1])

    ctx.file("BUILD.bazel", r"""package(default_visibility = ["//visibility:public"])
exports_files(glob(["**"]))
load("@rules_dotnet//dotnet/private/rules/nuget:nuget_archive.bzl", "tfm_filegroup", "rid_filegroup", "overrides")
""" + "\n".join([
        ...
        "filegroup(name = \"analyzers\", srcs = [%s])" % ",".join(["\n  \"%s\"" % a for a in analyzers]),
        "filegroup(name = \"data\", srcs = [])",
        _create_rid_native_select("native", groups.get("runtimes")) or "filegroup(name = \"native\", srcs = [])",
        "filegroup(name = \"content_files\", srcs = [%s])" % ",".join(["\n  \"%s\"" % a for a in content_files(groups.get("contentFiles")["any"])]),
        "overrides(name = \"overrides\", overrides = {overrides})".format(overrides = overrides),
    ]))

# this edge case is implemented for the host package builder & can be removed from monorepo onwards
def content_files(cfs):
    if cfs and cfs[0] == "contentFiles/any/net8.0/Metadata/metadata.json":
        return ["contentFiles/any/net8.0/Metadata"]
    ...
    for (tfm, rids) in tfm_rids.items():
        ...
        if "default" in rids:
            ...
            if next_tfm:
                ...
                map["//conditions:default"] = "%s_empty" % (name)
        native.alias(
...
overrides = rule(
    implementation = _overrides_impl,
    attrs = {
        "overrides": attr.string_dict(),
    },
    doc = "Targetting pack overrides provider.",
)
